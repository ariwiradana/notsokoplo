...
const HeroComponent = () => {
  ...
  useEffect(() => {
    function handleResize() {
      ...
      if (store.release.length > 0) {
        const r = store.release.find((x) => x.orientation === o);
        setRelease(r || null);
      }
    }
        ...
        <>
          ...
          >
            ...
            >
              <Button
                icon={
                  release.action_title ? <TbLink /> : <TbPlayerPlayFilled />
                }
                title={release.action_title || "Stream Sekarang"}
              />
            </Link>
          </div>
        </>
